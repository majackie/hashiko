...
Out of scope are real-time alerting, temporal correlation, login monitoring, process monitoring and rule-based detection. These exclusions distinguish Hashiko from comprehensive HIDS platforms like OSSEC. OSSEC includes rootkit detection and compliance reporting. Hashiko focuses on centralized hash collection and storage.
...
\hline
User-space agents; central server and storage & Kernel-level monitoring / memory scanning \\
\hline
File integrity (SHA-256) & Rootkit detection / kernel mods \\
\hline
HTTP transfer, queue-based retry & Real-time alerting / correlation \\
\hline
Web UI, APIs, indexed storage & Login/process monitoring \\
\hline
Deliverables: agent, server, APIs, database & Rule-based detection / forensic tooling \\
\hline
\end{tabular}
\end{table}
